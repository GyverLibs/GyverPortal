...
// ======= GPunix =======
int8_t GPgmt;   // переменная для установки часового пояса в часах (умолч. 3 - Москва)

struct GPunix;  // установка и хранение времени для графиков (занимает 4 байта памяти)

Внутри:
uint32_t unix;  // время в формате unix (секунды)

// установить время
void set(uint16_t y, uint8_t m, uint8_t d, uint8_t h, uint8_t mn, uint8_t s);

// добавить к текущему времени sec секунд
...
GPaddUnix(GPunix val, GPunix* arr, uint8_t am);     // новое значение, массив, размер массива
GPaddUnix(int16_t val, GPunix* arr, uint8_t am);    // добавить секунды!, массив, размер массива
```
</details>
...
add.PLOT_STOCK_DARK<к-во осей, к-во данных>(имя, подписи, массив времён, массив данных, int dec = 0)
```
Данный график требует для отображения массив даты и времени типа `GPunix`. Каждой точке можно вручную полностью присвоить дату и время через 
```cpp
set(uint16_t y, uint8_t m, uint8_t d, uint8_t h, uint8_t mn, uint8_t s);
```
Также можно указать текущую дату и время только для крайнего (последнего) элемента в массиве, а при последующих обновлениях по своему таймеру вызывать 
```cpp
GPaddUnix(к-во секунд с прошлого обновления, массив GPunix, размер массива);
```
