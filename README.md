...
```
Для подключения к порталу нужно зайти в браузере на IP адрес платы, который выдал ей роутер. В примере выше этот вдрес выводится в монитор порта.
...
- При нажатии на SUBMIT страница перезагружается, поэтому значения компонентов страницы нужно хранить в переменных и передавать при сборке страницы
...
- Клик позволяет получить текущее (или изменённое) значение только с кликнутого компонента. Страница не перезагрузится.
...
Вместо ручного опроса `click()` можно подключить свою функцию вида `void f(GyverPortal*)`, она будет вызвана при нажатиях на некоторые компоненты. 
...
- ПРОБЕЛ ПОСЛЕ ЗАПЯТОЙ СТАВИТЬ НЕЛЬЗЯ. 
