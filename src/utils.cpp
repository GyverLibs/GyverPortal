...
GPcolor::GPcolor() {}
GPcolor::GPcolor(uint32_t col) {
    setHEX(col);
}
GPcolor::GPcolor(uint8_t nr, uint8_t ng, uint8_t nb) {
    setRGB(nr, ng, nb);
}

void GPcolor::setRGB(uint8_t nr, uint8_t ng, uint8_t nb) {
    r = nr;
    g = ng;
    b = nb;
}

void GPcolor::operator = (uint32_t col) {
    setHEX(col);
}

void GPcolor::setHEX(uint32_t col) {
    r = ((uint32_t)col >> 16) & 0xFF;
    g = ((uint32_t)col >> 8) & 0xFF;
    b = col & 0xFF;
}

uint32_t GPcolor::getHEX() {
...
// struct DateTimeP {
    ...
    // uint8_t ss;     // 00ssssss
uint16_t DateTimeP::year() { return (ymdhm >> 19) & 0xfff; }
uint8_t DateTimeP::month() { return (ymdhm >> 15) & 0xf; }
uint8_t DateTimeP::day() { return (ymdhm >> 11) & 0xf; }
uint8_t DateTimeP::hour() { return (ymdhm >> 6) & 0x1f; }
uint8_t DateTimeP::minute() { return ymdhm & 0x3f; }
uint8_t DateTimeP::second() { return ss & 0x3f; }
void DateTimeP::year(uint16_t y) { ymdhm = (ymdhm & ~(0xffful << 19)) | ((uint32_t)(y & 0xfff) << 19); }
void DateTimeP::month(uint8_t m) { ymdhm = (ymdhm & ~(0xful << 15)) | ((uint32_t)(m & 0xf) << 15); }
void DateTimeP::day(uint8_t d) { ymdhm = (ymdhm & ~(0xf << 11)) | ((d & 0xf) << 11); }
void DateTimeP::hour(uint8_t h) { ymdhm = (ymdhm & ~(0x1f << 6)) | ((h & 0x1f) << 6); }
void DateTimeP::minute(uint8_t m) { ymdhm = ((ymdhm & (~0x3f)) | (m & 0x3f)); }
void DateTimeP::second(uint8_t s) { ss = s & 0x3f; }
void DateTimeP::set(uint16_t y, uint8_t m, uint8_t d, uint8_t h, uint8_t mn, uint8_t s) {
